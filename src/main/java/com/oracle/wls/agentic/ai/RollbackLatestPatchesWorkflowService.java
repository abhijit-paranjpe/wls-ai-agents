...
@Service.Singleton
public class RollbackLatestPatchesWorkflowService implements PatchingWorkflowPlanProvider {

    private static final WorkflowExecutionPlan PLAN = new WorkflowExecutionPlan(
            "rollback-latest-patches-workflow",
            List.of(
                    ...
                    new WorkflowExecutionStep(3,
                            "rollback-latest-patches",
                            "Rollback the latest applied patches for domain %s and return strict JSON with factual execution details and any tracking identifiers produced by tools.",
                            WorkflowStepAgentType.PATCHING,
                            null),
                    new WorkflowExecutionStep(4,
                            "monitor-patch-completion",
                            "Using tracking data from lastResponse, monitor patch rollback completion for domain %s and return strict JSON with factual status only.",
                            WorkflowStepAgentType.MONITORING,
                            "rollback-latest-patches"),
